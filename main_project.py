...
def launch_estimation(filename = None, continuous_to_categorical = None, target = None, epsilon = None):
    try:
        ...
        if continuous_to_categorical is None and not hasattr(sys.stdin, 'isatty') or sys.stdin.isatty(): 
            print('Any continuous attributes that can be considered categorical? Leave empty if none! (separate via comma)')
            continuous_to_categorical = input('Here: ')
        ...
        if epsilon is None and not hasattr(sys.stdin, 'isatty') or sys.stdin.isatty(): 
            print('With Differential Privacy (epsilon = 0.1) or without Differential Privacy (epsilon = 0)? ')
            infff = input("No DP - 0 || With DP - 1 :")
            if infff == '1':
                epsilon = 0.1
            elif infff == '0':
                epsilon = 0
